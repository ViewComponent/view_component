...
* Allow instrumentation to be automatically included in Server-Timing headers generated by Rails. To enable this set the config `config.instrumentation_use_deprecated_name = false`.  The old key `!render.view_component` is deprecated, please update your ActiveSupport::Notification subscriptions to `render.view_component`.
