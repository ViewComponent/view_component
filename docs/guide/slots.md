...
To provide content for a lambda slot via a block, add a block parameter. You can then render the content by calling the block's `call` method, or pass the block directly to `content_tag`:
