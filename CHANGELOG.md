...
* Add `ActionView::SlotableV2`
  ...
  * Slot definitions now accept either a component class, component class
    name, or a lambda instead of a `class_name:` keyword argument.
  * Slots now support positional arguments.
  * Slots no longer use the `content` attribute to render content, instead
    relying on `to_s`. e.g. `<%= my_slot %>`
  * Slot values are no longer set via the `slot` method, and instead use
